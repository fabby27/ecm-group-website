...
async function startServer() {
  ...
  if (!RESEND_API_KEY) {
    ...
    console.error("[ERROR] Email notifications will not work.");
  } else {
    console.log("[INFO] RESEND_API_KEY is configured");
  }
  ...
  app.post("/api/booking", async (req, res) => {
    try {
      ...
      if (!resend) {
        console.error("[Booking API] Cannot send email - RESEND_API_KEY not configured");
